...
import "../../css/CourseContent.css"; // Import your CSS for styling
...
const CourseContent = () => {
  ...
  const downloadPdfDocument = async (
    ...
    if (!input) return alert("Certificate element not found");
  ...
  const completeModule = async (sectionId) => {
    if (completedModule.length < courseContent.length) {
      ...
      if (
        ...
        !completedSections.includes(playingSectionIndex)
      ) {
                      ...
                      {section.S_content && (
                        <>
                          <Button
                            ...
                            onClick={() =>
                              playVideo(
                                `http://localhost:8000${section.S_content.path}`,
                                index
                              )
                            }>
                            Play Video
