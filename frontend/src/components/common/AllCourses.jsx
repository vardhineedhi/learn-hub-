...
import "../css/AllCourses.css"; // Importing the CSS file for styling
...
const AllCourses = () => {
  ...
  const [showModal, setShowModal] = useState(
    Array(allCourses.length).fill(false)
  );
  ...
  const handleSubmit = async (courseId) => {
    try {
      ...
      if (res.data.success) {
        ...
        navigate(
          `/courseSection/${res.data.course.id}/${res.data.course.Title}`
        );
      } else {
        alert(res.data.message);
        navigate(
        ...
        {allCourses?.length > 0 ? (
          allCourses
            .filter(
              (course) =>
                filterTitle === "" ||
                course.C_title?.toLowerCase().includes(
